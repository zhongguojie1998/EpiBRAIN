...
def smooth_gradients_input_attribution_diff(
    model,
    seq_input,
    output_key,
    target_pos_dim,
    ...
    """
    Compute smoothed gradient×input attribution for differential expression.
    ...
    using SmoothGrad (averaging gradients over noisy samples).
    ...
    Args:
        ...
        sample_prob: Probability of not mutating a position
        sample_value: New one-hot value for corrupted positions (default 1.0 for uniform, 0.25 for equal prob)
    ...
    # Loop over smoothgrad samples
    for sample_i in range(n_samples):
        # Create noisy version of input
        seq_input_sample = seq_input.clone()

        # Generate corruption mask: True means keep original, False means corrupt
        # Shape: [batch, 1, length] to broadcast across channels
        corruption_mask = rng.rand(seq_input.shape[0], 1, seq_input.shape[2]) < sample_prob
        corruption_mask = torch.from_numpy(corruption_mask).to(device)

        # For positions to corrupt, replace with sample_value
        # If sample_value=1.0, set to uniform [0.25, 0.25, 0.25, 0.25]
        # If sample_value=0.25, set to [0.25, 0.25, 0.25, 0.25]
        if sample_value == 1.0:
            # Uniform distribution across all nucleotides
            corrupted_values = torch.ones_like(seq_input_sample) * 0.25
        else:
            # Use specified value
            corrupted_values = torch.ones_like(seq_input_sample) * sample_value

        # Apply corruption mask
        seq_input_sample = torch.where(
            corruption_mask.expand_as(seq_input_sample),
            seq_input_sample,
            corrupted_values
        )

        # Enable gradient computation for input
        seq_input_sample.requires_grad_(True)

        # Forward pass
        output_dict = model(seq_input_sample)
        ...
        if grads_accumulated is None:
            ...
            grads_accumulated = grads_accumulated + grads

        # Clean up
        del seq_input_sample, output_dict, preds, preds_slice, preds_pos, preds_neg
        del preds_pos_agg, preds_neg_agg, score, grads

    # Average gradients across samples
    grads_accumulated = grads_accumulated / n_samples

    # Subtract mean across nucleotides at each position
    if subtract_avg:
        grads_accumulated = grads_accumulated - grads_accumulated.mean(dim=-1, keepdim=True)

    # Multiply by input (gradient × input)
    if input_gate:
        seq_input_permuted = seq_input.permute(0, 2, 1)  # [batch, length, channels]
        grads_accumulated = grads_accumulated * seq_input_permuted
...
def process_region_chunk(args):
    ...
    param_suffix = f"sp{sample_prob:.2f}_sv{sample_value:.2f}_pc{pseudo_count:.1f}"
    ...
    logger = BaseLogger(name=f"SmoothGradInputDiff-{device}", level=logging.INFO)
    ...
    for idx in range(len(region_chunk_data)):
        ...
        name_base = (
            f"{prefix}_{chr_name}_{start}_{end}_{gene_name}_{trial_pos_clean}_{trial_neg_clean}_{param_suffix}"
            if prefix is not None
            else f"{chr_name}_{start}_{end}_{gene_name}_{trial_pos_clean}_{trial_neg_clean}_{param_suffix}"
        )
        ...
        identifier = f"{name_base}_smooth_grad_input"
        if not os.path.exists(f"{save_base}/interp_diff_smooth_gradient_input/{identifier}.pt") or force_restart:
            nan_occur = False
            ...
            for rev_comp in [False, True] if rc else [False]:
                ...
                # Calculate smoothed gradient attribution for differential expression
            ...
            if save_raw:
                torch.save(attribution_cpu, f"{save_base}/interp_diff_smooth_gradient_input/{identifier}.pt")
        else:
            attribution_cpu = torch.load(f"{save_base}/interp_diff_smooth_gradient_input/{identifier}.pt")
        ...
        if not no_plot:
            if nan_occur:
                ...
                plot_title.append(f"Importance Score (SmoothGrad×Input Diff)")
            ...
            plt.tight_layout()
            plt.savefig(
                f"{res_base}/{exp_name}/analysis_{chk}/plot/interp_diff_smooth_gradient_input/{name_base}_smooth_grad_input.png",
                dpi=300,
                bbox_inches="tight",
            )
            plt.close()
...
@click.option("--n_samples", type=int, default=5, help="Number of noisy samples for SmoothGrad")
@click.option("--sample_prob", type=float, default=0.90, help="Probability of not mutating a position")
@click.option("--sample_value", type=float, default=1.0, help="New one-hot value for corrupted positions (1.0=uniform, 0.25=equal)")
@click.option("--sample_seed", type=int, default=42, help="Random seed for SmoothGrad")
def main(
    region_bed,
    exp_name,
    chk,
    ...
    os.makedirs(f"{RES_BASE}/{exp_name}/analysis_{chk}/plot/interp_diff_smooth_gradient_input", exist_ok=True)
    os.makedirs(f"{RES_BASE}/{exp_name}/analysis_{chk}/raw_data/interp_diff_smooth_gradient_input", exist_ok=True)
    os.makedirs(f"{RES_BASE}/{exp_name}/analysis_{chk}/raw_data/label", exist_ok=True)

    logger = BaseLogger(name="SmoothGradInputDiff", level=logging.INFO)
    ...
    for i in range(len(region_df)):
        ...
        if no_plot:
            ...
            output_files_exist = (
                os.path.exists(f"{RES_BASE}/{exp_name}/analysis_{chk}/raw_data/interp_diff_smooth_gradient_input/{name_base}_smooth_grad_input.pt") and
            ...
            output_file = f"{RES_BASE}/{exp_name}/analysis_{chk}/plot/interp_diff_smooth_gradient_input/{name_base}_smooth_grad_input.png"
            output_files_exist = (
