...
def process_region_chunk(args):
    ...
    # Compiled models can interfere with gradient-based attribution methods like DeepLift
    if myconfig.model.get("use_compile", False):
        logger.info("Disabling model compilation for interpretation (not compatible with attribution methods)")
    ...
    for idx in range(len(region_chunk_data)):
        ...
        # Keep plus/minus suffix for foreground
        trial_pos_clean = clean_trial_name(trial_pos, keep_suffix=True)
        # For background, just use count
        neg_trial_count = len(trial_neg.split(';'))
        trial_neg_clean = f"other-{neg_trial_count}"

        # trial_neg is now always populated from BED file (background celltypes)
        name_base = (
        ...
        # trial_neg is now a semicolon-separated string from BED file
        ...
        # Parse bin_range from BED file instead of calculating from coordinates
