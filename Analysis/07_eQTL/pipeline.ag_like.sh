...
LOG_DIR="${SCRIPT_DIR}/logs.ag_like"
mkdir -p "$LOG_DIR"

RUN="python ${SCRIPT_DIR}/run.py"

JSON="${SCRIPT_DIR}/data_paths.ag_like.json"
JSON_FLAG="--json ${JSON}"
...
nohup $RUN evaluate $JSON_FLAG --model alphagenome_524k                         > "$LOG_DIR/alphagenome_524k.log" 2>&1 &
nohup $RUN evaluate $JSON_FLAG --model alphagenome_524k --filter brain          > "$LOG_DIR/alphagenome_524k_brain.log" 2>&1 &
nohup $RUN evaluate $JSON_FLAG --model alphagenome_524k --filter basal_ganglia  > "$LOG_DIR/alphagenome_524k_basal_ganglia.log" 2>&1 &
nohup $RUN evaluate $JSON_FLAG --model alphagenome_524k --filter cortex         > "$LOG_DIR/alphagenome_524k_cortex.log" 2>&1 &
nohup $RUN evaluate $JSON_FLAG --model alphagenome_524k --filter gtex           > "$LOG_DIR/alphagenome_524k_gtex.log" 2>&1 &
