...
def process_region_chunk(args):
    ...
    dl_model = DeepLift(model.eval().to(device), multiply_by_inputs=True, eps=1e-7)
    ...
    for idx in range(len(region_chunk_data)):
        ...
        identifier = f"{chr_name}_{start}_{end}_{trial}"
        ...
        if (
            not os.path.exists(f"{res_base}/{exp_name}/analysis_{chk}/raw_data/interp/{identifier}.pt")
            or force_restart
        ):
            all_attribution = []
            nan_occur = False
            ...
            torch.save(
                all_attribution,
                f"{res_base}/{exp_name}/analysis_{chk}/raw_data/interp/{identifier}.pt",
            )
        else:
            all_attribution = torch.load(f"{res_base}/{exp_name}/analysis_{chk}/raw_data/interp/{identifier}.pt")
...
@click.option("--num_processes", type=int, default=1, help="Number of subprocess to use for parallel processing")
def main(
    region_bed, exp_name, chk, baseline, log_base, chk_base, res_base, force_restart, processor, num_processes
):
